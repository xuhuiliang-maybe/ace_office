...
@class_view_decorator(permission_required('increase_info.change_increase', raise_exception=True))
...
class IncreaseUpdate(SuccessMessageMixin, UpdateView):
	...
	def get_success_url(self):
		...
		if _addanother:
			self.url = reverse('increase_info:increase_add')
		return self.url

	# 增加返回参数
	def get_context_data(self, **kwargs):
		context = super(IncreaseUpdate, self).get_context_data(**kwargs)
		context["form_content"] = u"修改增员信息"
...
@class_view_decorator(permission_required('increase_info.change_increase', raise_exception=True))
@class_view_decorator(check_principal)  # 校验是否负责该项目
class IncreaseUpdateRemark(SuccessMessageMixin, UpdateView):
	"""编辑备注信息
	"""
	template_name = "base/document_edit.html"
	model = Increase
	...
	# 增加返回参数
	def get_context_data(self, **kwargs):
		context = super(IncreaseUpdateRemark, self).get_context_data(**kwargs)
		context["form_content"] = u"增员信息到期，只提供添加备注"
		referrer = self.request.META.get('HTTP_REFERER', "")
		context["referrer"] = referrer
		return context
...
@class_view_decorator(login_required)
@class_view_decorator(permission_required('increase_info.change_increase', raise_exception=True))
class IncreaseCustomUpdate(View):
	"""编辑单个字段，部门确认，最终确认
	"""
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			...
			pk_str = request.POST.get("pk", "-1")  # 以逗号分割，兼容批量确认
			...
			if oper_type == "batch_oper":
				for one_pk in pk_list:
					...
					if filed == "dept_verify":
						if one_pk:
							increase_obj = Increase.objects.filter(id=one_pk)
						try:
							if increase_obj.exists():
								check_pass = 0
								...
								if increase_obj[0].emplyid.project_name.department not in all_customer_service_dept:
									#  当前信息部门，不属于客服部
								...
								# 客服经理
								if not check_pass:
									customer_service_director = increase_obj[0].emplyid.project_name.customer_service_director
					...
					Increase.objects.filter(id=one_pk).update(**kwargs)
			...
			if not oper_type:
				...
				# 部门确认：只能由员工部门经理来操作，dept_verify
				if filed == "dept_verify":
					increase_obj = Increase.objects.filter(id=pk_list[0])
					try:
						if increase_obj.exists():
							check_pass = 0
							# 非客服部的项目，由人事部负责人做部门确认
							...
							# 客服经理
							if not check_pass:
								customer_service_director = increase_obj[
				...
				kwargs = get_kwargs(search_condition)
				Increase.objects.filter(id=pk_list[0]).update(**kwargs)
				result["msg"] = u"成功设置"
				result["code"] = 1
		except:
			traceback.print_exc()
