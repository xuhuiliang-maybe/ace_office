...
class ArchiveList(ListView):
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_superuser:
            ...
            search_condition.update({"employee_id__project_name__department__id__in": managements})

        # 普通管理员只查询自己负责项目的员工信息
        if not self.request.user.is_superuser and not self.request.user.dept_head:
            search_condition.update({"employee_id__project_name__principal": self.request.user})
