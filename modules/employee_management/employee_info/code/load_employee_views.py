					...
					try:
						...
						if emp_obj.exists():
							...
							emp_obj.update(
								...
								bank_address=row[8],  # 开户地
								project_name=project_name,
								birthday=get_excel_date(row[14]),  # 出生年月
								entry_date=get_excel_date(row[24]),  # 入职日期
								call_out_time=get_excel_date(row[25]),  # 调出日期
								into_time=get_excel_date(row[26]),  # 转入日期

								social_insurance_increase_date=get_excel_date(row[27]),  # 社保增员日期
								provident_fund_increase_date=get_excel_date(row[31]),  # 公积金增员日期
								contract_begin_date=get_excel_date(row[33]),  # 合同开始日期
								departure_date=get_excel_date(row[38]),  # 离职日期
								social_insurance_reduce_date=get_excel_date(row[41]),  # 社保减员日期
								business_insurance_reduce_date=get_excel_date(row[42]),  # 商保减员日期
								provident_fund_reduce_date=get_excel_date(row[43]),  # 公积金减员日期
								age=get_excel_int(row[15]),  # 员工年龄
								register_address=row[16],  # 户口所在地
								register_postcode=str(get_excel_int(row[17], True)),  # 户口邮编
							)
							continue
						...
						create_result = Employee.objects.create(
							...
							salary_card_number=row[6],  # 银行卡号
							bank_account=row[7],  # 开户银行
							bank_address=row[8],  # 开户银行
							job_dept=row[9],  # 部门
							position=row[10],  # 职务
							sex=gender_dict.get(row[11], ""),  # 性别
							nation=nation_dict.get(row[12], "1"),  # 民族
							education=education_dict.get(row[13], ""),  # 学历
							birthday=get_excel_date(row[14]),  # 出生年月
							age=get_excel_int(row[15]),  # 员工年龄
							register_address=row[16],  # 户口所在地
							register_postcode=str(get_excel_int(row[17], True)),  # 户口邮编
							register_type=registertype_dict.get(row[18], ""),  # 户口性质
							work_address=row[19],  # 工作地
							insured_place=row[20],  # 社保地
							person_type=ContractType.get_dict_item_by_name(row[21]),  # 人员属性
							contract_type=contract_type_dict.get(row[22], ""),  # 合同属性
							contract_subject=CompanySubject.get_dict_item_by_name(row[23]),  # 合同主体
							entry_date=get_excel_date(row[24]),  # 入职日期
							call_out_time=get_excel_date(row[25]),  # 调出日期
							into_time=get_excel_date(row[26]),  # 转入日期
							social_insurance_increase_date=get_excel_date(row[27]),  # 社保增员日期
							social_security_payment_card=row[28],  # 社保支付卡
							use_bank=row[29],  # 开户银行
							business_insurance_increase_date=get_excel_date(row[30]),  # 商保增员日期
							provident_fund_increase_date=get_excel_date(row[31]),  # 公积金增员日期
							contract_begin_date=get_excel_date(row[32]),  # 合同开始日期
							probation_period=get_excel_int(row[33]),  # 试用期限
							contract_period=get_excel_int(row[34]),  # 合同期限
							probation_end_date=get_excel_date(row[35]),  # 试用期日期
							contract_end_date=get_excel_date(row[36]),  # 合同到期日期
							contract_renew_times=get_excel_int(row[37]),  # 合同续签次数
							departure_date=get_excel_date(row[38]),  # 离职日期
							departure_procedure=departure_procedure_dict.get(row[39], ""),  # 离职手续
							departure_cause=row[40],  # 离职原因
							social_insurance_reduce_date=get_excel_date(row[42]),  # 社保减员日期
							business_insurance_reduce_date=get_excel_date(row[42]),  # 商保减员日期
							provident_fund_reduce_date=get_excel_date(row[43]),  # 公积金减员日期
							phone_number=str(get_excel_int(row[44], True)),  # 联系电话
							contact_person=row[45],  # 紧急联系人
							contact_relationship=row[46],  # 与联系人关系
							contact_person_phone=str(get_excel_int(row[47], True)),  # 紧急联系人电话
							recruitment_channel=recruitment_channel_dict.get(row[48], ""),  # 招聘渠道
							recruitment_attache=Profile.get_user_by_username_or_first_name(row[49]),  # 招聘人员
						)
						import_num += 1
					except:
