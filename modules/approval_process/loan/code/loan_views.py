...
@class_view_decorator(permission_required('loan.browse_loan', raise_exception=True))
class LoanListView(ListView):
	...
	def get_context_data(self, **kwargs):
		...
		context["apply_type"] = "loan"
		context["add_url"] = reverse('approval:loan_add')  # 新增url
...
@class_view_decorator(login_required)
@class_view_decorator(permission_required('loan.browse_loan', raise_exception=True))
class LoanBudgetDetailsListView(ListView):
	context_object_name = "loan_budget_details_list"
	template_name = "loan_budget_details_list.html"
	allow_empty = True
	paginate_by = PAGINATE
	...
	def get_queryset(self):
		try:
			...
			kwargs = get_kwargs(search_condition)
			return LoanBudgetDetails.objects.filter(**kwargs)
		except:
			traceback.print_exc()

	def get_context_data(self, **kwargs):
		context = super(LoanBudgetDetailsListView, self).get_context_data(**kwargs)
		context["add_url"] = reverse('approval:loan_budget_details_add', args=(self.applicants,))  # 新增url
		# 1:由明细表返回，申请页面带查询条件，2：由待审批超链进去，不带查询条件
		context["return_url"] = reverse('approval:loan_one', args=(self.applicants, self.view_type))  # 返回url
		context["view_type"] = self.view_type
		context["applicants"] = self.applicants
		context["apply_info"] = Loan.objects.filter(id=self.applicants).first()
		return context
