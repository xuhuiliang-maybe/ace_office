...
@class_view_decorator(permission_required('billing_pre_pay.add_billingprepay', raise_exception=True))
class BillingPrePayCreate(SuccessMessageMixin, CreateView):
	model = BillingPrePay
	...
	def get_context_data(self, **kwargs):
		context = super(BillingPrePayCreate, self).get_context_data(**kwargs)
		referrer = self.request.META.get('HTTP_REFERER', "")
		context["referrer"] = referrer
		context["form_content"] = u"结算与发薪"
		return context

	# 将登陆用户作为申请人
	def form_valid(self, form):
		form.instance.apply_user = self.request.user
		form.instance.apply_types = "6"
		...
		return super(BillingPrePayCreate, self).form_valid(form)
