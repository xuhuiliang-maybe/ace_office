...
@class_view_decorator(permission_required('billing_pre_pay.browse_billingprepay', raise_exception=True))
class BillingPrePayListView(ListView):
	context_object_name = "billing_pre_pay_list"
	...
	def get_queryset(self):
		try:
			...
			return BillingPrePay.objects.filter(**kwargs)
		except:
			traceback.print_exc()

	def get_context_data(self, **kwargs):
		context = super(BillingPrePayListView, self).get_context_data(**kwargs)
		context["title"] = self.title
		context["status"] = self.status
		context["month"] = self.month  # 申请月份
		context["dept_name"] = self.dept_name  # 部门
		context["project_name"] = self.project_name  # 项目名称
		context["apply_type"] = "billing_pre_pay"
		context["add_url"] = reverse('approval:billing_pre_pay_add')  # 新增url
