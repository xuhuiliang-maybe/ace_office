...
@class_view_decorator(permission_required('contract_manage.add_contractservice', raise_exception=True))
class ContractServiceCreate(SuccessMessageMixin, CreateView):
	form_class = ContractServiceForm
	template_name = "contract_edit.html"
	success_message = u"%(name)s 创建成功"

	def get_context_data(self, **kwargs):
		context = super(ContractServiceCreate, self).get_context_data(**kwargs)
	...
	def form_valid(self, form):
		...
		return super(ContractServiceCreate, self).form_valid(form)
