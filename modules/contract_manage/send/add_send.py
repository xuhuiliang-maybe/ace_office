...
@class_view_decorator(permission_required('contract_manage.add_contractsend', raise_exception=True))
class ContractSendCreate(SuccessMessageMixin, CreateView):
	form_class = ContractSendForm
	template_name = "contract_edit.html"
	success_message = u"%(name)s 创建成功"

	def get_context_data(self, **kwargs):
		context = super(ContractSendCreate, self).get_context_data(**kwargs)
	...
	def form_valid(self, form):
		...
		return super(ContractSendCreate, self).form_valid(form)
